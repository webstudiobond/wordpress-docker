...
### `http` Context:
- `limit_zones.conf` (`limit_zones.conf.example`) — Rate-limiting zone definitions (`limit_req_zone`, `limit_conn_zone`)
...
- `limit_all.conf` (`limit_all.conf.example`) — Rate limiting enforcement (`limit_req`, etc.)
